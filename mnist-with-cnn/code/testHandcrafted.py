...
for i in range(len(dataTypes)):
    ...
    for j in range(len(featureTypes)):
        ...
        print(
            "{:.2f}s to extract {} features ({} dim).".format(
                time.time() - tic, featureType, features.shape[0]
            )
        )
        ...
        model = linearModel.train(
            features[:, data["set"] == trainSet], data["y"][data["set"] == trainSet]
        )
